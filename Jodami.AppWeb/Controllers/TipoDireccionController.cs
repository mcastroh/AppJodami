...
namespace Jodami.AppWeb.Controllers
{
    public class TipoDireccionController : Controller
    {
        ...
        private readonly IGenericService<TipoDireccion> _service;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly Usuario _sessionUsuario;

        #region Constructor 

        public TipoDireccionController(IMapper mapper, IGenericService<TipoDireccion> service, IHttpContextAccessor httpContextAccessor)
        {
            _mapper = mapper;
            _service = service;
        ...
        {
            var query = _mapper.Map<List<VMTipoDireccion>>(await _service.GetAll());
            return View(query);
        }
        ...
        public async Task<IActionResult> Editar(VMTipoDireccion vmModelo)
        {
            var modelo = await _service.GetById(x => x.IdTipoDireccion == vmModelo.IdTipoDireccion);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Eliminar(VMTipoDireccion vmModelo)
        {
            bool flgRetorno = await _service.Delete(x => x.IdTipoDireccion == vmModelo.IdTipoDireccion);
            return RedirectToAction("Index");
        }
        ...
        public async Task<IActionResult> ListarPDF()
        {
            ...
            string titulo = "Tipos de Direcciones";
            ...
            {
                FileName = $"Tipos Direcciones {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}.pdf",
