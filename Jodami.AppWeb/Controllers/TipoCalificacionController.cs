...
namespace Jodami.AppWeb.Controllers
{
    public class TipoCalificacionController : Controller
    {
        ...
        private readonly IGenericService<TipoCalificacion> _service;
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly Usuario _sessionUsuario;

        #region Constructor 

        public TipoCalificacionController(IMapper mapper, IGenericService<TipoCalificacion> service, IHttpContextAccessor httpContextAccessor)
        {
            _mapper = mapper;
            _service = service;
        ...
        {
            var query = _mapper.Map<List<VMTipoCalificacion>>(await _service.GetAll());
            return View(query);
        }
        ...
        {
            var modelo = new TipoCalificacion()
            {
        ...
        public async Task<IActionResult> Editar(VMTipoCalificacion vmModelo)
        {
            var modelo = await _service.GetById(x => x.IdTipoCalificacion == vmModelo.IdTipoCalificacion);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Eliminar(VMTipoCalificacion vmModelo)
        {
            bool flgRetorno = await _service.Delete(x => x.IdTipoCalificacion == vmModelo.IdTipoCalificacion);
            return RedirectToAction("Index");
        }
        ...
        public async Task<IActionResult> ListarPDF()
        {
            ...
            string titulo = "Tipos de Calificación";
            ...
            {
                FileName = $"Tipos de Calificacion {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}.pdf",
