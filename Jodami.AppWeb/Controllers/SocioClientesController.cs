...
{
    public class SocioClientesController : Controller
    {
        #region Variables 
        ...
        public SocioClientesController(
        ...
        public async Task<IActionResult> ListarPDF()
        {
            ...
            {
                FileName = $"Clientes {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}.pdf",
        ...
        #region ETL => Nuevo/Editar/Eliminar Cliente
        ...
        private async Task<VMSociosProveedores> Etl_SociosProveedores(int idSocio)
        {
            ...
            var tipoSocioClientes = await _contexto.TipoSocio.AsNoTracking().FirstOrDefaultAsync(x => x.Codigo == KeysNames.CLIENTES);
            ...
            if (idSocio == 0)
            {
                ...
                {
                    IdTipoSocio = tipoSocioClientes.IdTipoSocio,
        ...
        public async Task<List<VMSociosProveedores>> ETL_SociosComerciales()
        {
            ...
            var entityGruposEconomicos = _mapper.Map<List<VMSociosGrupos>>(await _contexto.Socio.AsNoTracking().Where(x => x.IdTipoSocio == tipoSocioGruposEconomicos.IdTipoSocio).ToListAsync());
            var entityColaboradores = _mapper.Map<List<VMSociosColaboradores>>(await _contexto.Socio.AsNoTracking().Where(x => x.IdTipoSocio == tipoSocioColaboradores.IdTipoSocio).ToListAsync());
            var entityProveedores = _mapper.Map<List<VMSociosProveedores>>(await _contexto.Socio.AsNoTracking().Where(x => x.IdTipoSocio == tipoSocioClientes.IdTipoSocio).ToListAsync());
            ...
            ViewBag.TipoSocio = tipoSocioClientes;
